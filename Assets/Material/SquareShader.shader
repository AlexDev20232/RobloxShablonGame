    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert(appdata v)
            {
                ...
                o.uv = v.uv;
                o.overlayUV = TRANSFORM_TEX(v.uv, _MainTex);
                o.scaleXZ = float2(length(axisX), length(axisZ));
                return o;
            }

            fixed4 frag(v2f i) : SV_Target
            {
                float2 scaleXZ = max(i.scaleXZ, 0.0001);
                float2 scaledUV = (i.uv * scaleXZ) * max(_GridScale, 0.0001);
                ...
                fixed4 texCol = tex2D(_MainTex, i.overlayUV);
